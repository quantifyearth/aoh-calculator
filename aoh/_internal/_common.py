...
def single_layer(path_or_constant: Path | str | float | int ) -> yg.YirgacheffeLayer | float:
    # The structure in here is a bit odd - originally this was just a command line tool
    # what morphed into a library, and so this can be constants or file paths. Ideally there
    # would be another abstraction layer so that the aohcalc_* methods took yg.YirgacheffeLayer
    if isinstance(path_or_constant, (int, float)):
        return path_or_constant
    else:
        try:
...
def load_weights(weight_layer_paths: list[ Path | str | float | int ] | None) -> yg.YirgacheffeLayer | float | None:
