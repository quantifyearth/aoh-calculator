...
def model_validation(aoh_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for klass in klasses:
        ...
        coef_df = extract_fixed_effects_coefficients(model, klass)
        per_class_model_coefficients.append(coef_df)

        ranef_df = extract_random_effects_coeffients(model, klass)
        ...
        klass_outliers = add_diagnostic_columns(klass_outliers, q3, q1, iqr)
...
def extract_fixed_effects_coefficients(model: "pymer4.models.Lmer", class_name: str) -> pd.DataFrame:
...
def extract_random_effects_coeffients(model: "pymer4.models.Lmer", class_name: str) -> pd.DataFrame:
...
def build_coefficients_pivot(coefficients: pd.DataFrame) -> pd.DataFrame:
    """Pivot fixed-effect coefficients to wide form by class and variable."""
    return coefficients.pivot(index='class_name', columns='variable', values='Estimate')


def generate_summary(output_dir: Path, aoh_df: pd.DataFrame, outliers: pd.DataFrame) -> None:
    """Generate a summary.md summarizing the model validation outputs."""
...
def validate_map_prevalence(
    ...
    # Save csvs useful for model diagnostics
    ...
    generate_summary(output_dir, aoh_df, outliers)
